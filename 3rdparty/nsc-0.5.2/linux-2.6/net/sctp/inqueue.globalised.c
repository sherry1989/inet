# 1 "linux-2.6/net/sctp/inqueue.c"
# 1 "/home/cmcc/omnet_workspace/inet/3rdparty/nsc-0.5.2//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "linux-2.6/net/sctp/inqueue.c"
# 46 "linux-2.6/net/sctp/inqueue.c"
...
# 47 "linux-2.6/net/sctp/inqueue.c" 2
...
# 48 "linux-2.6/net/sctp/inqueue.c" 2
...
void sctp_inq_init(struct sctp_inq *queue)
{
 ...
 do { do { (&(&queue->immediate)->entry)->next = (&(&queue->immediate)->entry); (&(&queue->immediate)->entry)->prev = (&(&queue->immediate)->entry); } while (0); (&queue->immediate)->pending = 0; do { ((&queue->immediate))->func = (((void *)0)); ((&queue->immediate))->data = (((void *)0)); } while (0); init_timer(&(&queue->immediate)->timer); } while (0);
...
struct sctp_inq *sctp_inq_new(void)
{
 ...
 retval = (struct sctp_inq *)kmalloc(sizeof(struct sctp_inq), (0x20));
 if (retval) {
...
void sctp_inq_free(struct sctp_inq *queue)
{
 ...
 while ((chunk = (struct sctp_chunk *) skb_dequeue(&queue->in)) != ((void *)0))
...
struct sctp_chunk *sctp_inq_pop(struct sctp_inq *queue)
{
 ...
 if (!chunk) {
  ...
  + ((((__builtin_constant_p((__u16)(( __u16)(__be16)(ch->length))) ? ({ __u16 __x = ((( __u16)(__be16)(ch->length))); ((__u16)( (((__u16)(__x) & (__u16)0x00ffU) << 8) | (((__u16)(__x) & (__u16)0xff00U) >> 8) )); }) : __fswab16((( __u16)(__be16)(ch->length)))))+3)&~3);
...
# 190 "linux-2.6/net/sctp/inqueue.c"
void sctp_inq_set_th_handler(struct sctp_inq *q,
     void (*callback)(void *), void *arg)
{
 do { do { (&(&q->immediate)->entry)->next = (&(&q->immediate)->entry); (&(&q->immediate)->entry)->prev = (&(&q->immediate)->entry); } while (0); (&q->immediate)->pending = 0; do { ((&q->immediate))->func = (callback); ((&q->immediate))->data = (arg); } while (0); init_timer(&(&q->immediate)->timer); } while (0);
}
