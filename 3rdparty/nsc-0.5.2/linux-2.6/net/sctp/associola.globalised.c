# 1 "linux-2.6/net/sctp/associola.c"
# 1 "/home/cmcc/omnet_workspace/inet/3rdparty/nsc-0.5.2//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "linux-2.6/net/sctp/associola.c"
# 51 "linux-2.6/net/sctp/associola.c"
...
# 52 "linux-2.6/net/sctp/associola.c" 2
...
# 53 "linux-2.6/net/sctp/associola.c" 2
...
# 10 "linux-2.6/include/linux/poll.h" 2
...
# 11 "linux-2.6/include/linux/poll.h" 2
...
# 12 "linux-2.6/include/linux/poll.h" 2
...
# 54 "linux-2.6/net/sctp/associola.c" 2
...
# 59 "linux-2.6/net/sctp/associola.c" 2
...
# 60 "linux-2.6/net/sctp/associola.c" 2
...
# 61 "linux-2.6/net/sctp/associola.c" 2
...
# 62 "linux-2.6/net/sctp/associola.c" 2
       ...
       int gfp)
{
 ...
 sp = (&((struct sctp_sock *)(struct sock *)sk)->sctp);
 ...
 do { (void)(&asoc->base.addr_lock); } while(0);
 ...
 for (i = SCTP_EVENT_TIMEOUT_NONE; i < (SCTP_EVENT_TIMEOUT_AUTOCLOSE + 1); ++i) {
  ...
  asoc->timers[i].function = (*_GLOBAL_sctp_timer_events_36_A[get_stack_id()])[i];
 ...
 asoc->unack_data = 0;
                          ...
                          ;
# 232 "linux-2.6/net/sctp/associola.c"
 asoc->addip_serial = asoc->c.initial_tsn;

 skb_queue_head_init(&asoc->addip_chunks);


 do { (&asoc->peer.transport_addr_list)->next = (&asoc->peer.transport_addr_list); (&asoc->peer.transport_addr_list)->prev = (&asoc->peer.transport_addr_list); } while (0);
# 250 "linux-2.6/net/sctp/associola.c"
 ...
 do { (&asoc->asocs)->next = (&asoc->asocs); (&asoc->asocs)->prev = (&asoc->asocs); } while (0);
...
# 416 "linux-2.6/net/sctp/associola.c"
        ...
        int gfp)
{
 ...
 if (asoc->pmtu)
  asoc->pmtu = ({ int __x = (peer->pmtu); int __y = (asoc->pmtu); __x < __y ? __x: __y; });
 else
  asoc->pmtu = peer->pmtu;
 ...
 sctp_packet_init(&peer->packet, peer, asoc->base.bind_addr.port,
    asoc->peer.port);
# 493 "linux-2.6/net/sctp/associola.c"
 peer->cwnd = ({ typeof(4*asoc->pmtu) _x = (4*asoc->pmtu); typeof(({ __u32 __x = (2*asoc->pmtu); __u32 __y = (4380); __x > __y ? __x: __y; })) _y = (({ __u32 __x = (2*asoc->pmtu); __u32 __y = (4380); __x > __y ? __x: __y; })); (void) (&_x == &_y); _x < _y ? _x : _y; });
    ...
    const union sctp_addr *addr)
{
 ...
 struct sctp_transport *peer = ((void *)0);
 struct sctp_transport *transport;
      ...
      sctp_sn_error_t error)
{
 ...
 event = sctp_ulpevent_make_peer_addr_change(asoc,
    ...
    0, spc_state, error, (0x20));
 if (event)
  sctp_ulpq_tail_event(&asoc->ulpq, event);
# 641 "linux-2.6/net/sctp/associola.c"
...
# 666 "linux-2.6/net/sctp/associola.c"
          ...
          __u32 tsn)
{
 ...
 struct sctp_transport *transport;
 struct sctp_chunk *chunk;
 __u32 key = (( __be32)(__builtin_constant_p((__u32)((tsn))) ? ({ __u32 __x = (((tsn))); ((__u32)( (((__u32)(__x) & (__u32)0x000000ffUL) << 24) | (((__u32)(__x) & (__u32)0x0000ff00UL) << 8) | (((__u32)(__x) & (__u32)0x00ff0000UL) >> 8) | (((__u32)(__x) & (__u32)0xff000000UL) >> 24) )); }) : __fswab32(((tsn)))));

 match = ((void *)0);
# 801 "linux-2.6/net/sctp/associola.c"
 active = asoc->peer.active_path;

 for (entry = (&active->transmitted)->next, prefetch(entry->next); entry != (&active->transmitted); entry = entry->next, prefetch(entry->next)) {
 ...
 for (pos = (&asoc->peer.transport_addr_list)->next, prefetch(pos->next); pos != (&asoc->peer.transport_addr_list); pos = pos->next, prefetch(pos->next)) {
  ...
  if (transport == active)
   break;
  for (entry = (&transport->transmitted)->next, prefetch(entry->next); entry != (&transport->transmitted); entry = entry->next, prefetch(entry->next)) {
   chunk = ({ const typeof( ((struct sctp_chunk *)0)->transmitted_list ) *__mptr = (entry); (struct sctp_chunk *)( (char *)__mptr - ((size_t) &((struct sctp_chunk *)0)->transmitted_list) );})
                         ;
   if (key == chunk->subh.data_hdr->tsn) {
    match = transport;
...
static void sctp_assoc_bh_rcv(struct sctp_association *asoc)
{
 ...
 while (((void *)0) != (chunk = sctp_inq_pop(inqueue))) {
  ...
  if ((chunk->chunk_hdr->type == SCTP_CID_DATA))
   ...
   (((*_GLOBAL_sctp_statistics_32_A[get_stack_id()])[!(((current_thread_info()->preempt_count) & (((1UL << (8))-1) << (0 + 8))))])->mibs[SCTP_MIB_INCTRLCHUNKS]++);
         ...
         struct sctp_association *new)
{
 ...
 if (asoc->state >= SCTP_STATE_ESTABLISHED) {
  ...
  for (pos = (&new->peer.transport_addr_list)->next, prefetch(pos->next); pos != (&new->peer.transport_addr_list); pos = pos->next, prefetch(pos->next)) {
   trans = ({ const typeof( ((struct sctp_transport *)0)->transports ) *__mptr = (pos); (struct sctp_transport *)( (char *)__mptr - ((size_t) &((struct sctp_transport *)0)->transports) );})
                   ;
 ...
 case SCTP_STATE_SHUTDOWN_SENT:
  if ((asoc->rwnd > asoc->a_rwnd) &&
      ((asoc->rwnd - asoc->a_rwnd) >=
       ({ __u32 __x = ((asoc->base.sk->sk_rcvbuf >> 1)); __u32 __y = (asoc->pmtu); __x < __y ? __x: __y; })))
   return 1;
  break;
 default:
  break;
...
{
 int var_size2 = (__builtin_constant_p((__u16)(( __u16)(__be16)(cookie->peer_init->chunk_hdr.length))) ? ({ __u16 __x = ((( __u16)(__be16)(cookie->peer_init->chunk_hdr.length))); ((__u16)( (((__u16)(__x) & (__u16)0x00ffU) << 8) | (((__u16)(__x) & (__u16)0xff00U) >> 8) )); }) : __fswab16((( __u16)(__be16)(cookie->peer_init->chunk_hdr.length))));
       ...
       const union sctp_addr *laddr)
{
 ...
 do { do { } while (0); do { (void)(&asoc->base.addr_lock); } while(0); (void)0; } while(0);
 if ((asoc->base.bind_addr.port == (__builtin_constant_p((__u16)(( __u16)(__be16)(laddr->v4.sin_port))) ? ({ __u16 __x = ((( __u16)(__be16)(laddr->v4.sin_port))); ((__u16)( (((__u16)(__x) & (__u16)0x00ffU) << 8) | (((__u16)(__x) & (__u16)0xff00U) >> 8) )); }) : __fswab16((( __u16)(__be16)(laddr->v4.sin_port))))) &&
