# 1 "linux-2.6/net/sctp/ulpqueue.c"
# 1 "/home/cmcc/omnet_workspace/inet/3rdparty/nsc-0.5.2//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "linux-2.6/net/sctp/ulpqueue.c"
# 44 "linux-2.6/net/sctp/ulpqueue.c"
...
# 45 "linux-2.6/net/sctp/ulpqueue.c" 2
...
# 46 "linux-2.6/net/sctp/ulpqueue.c" 2
...
# 47 "linux-2.6/net/sctp/ulpqueue.c" 2
# 1 "linux-2.6/include/net/sctp/structs.h" 1
# 59 "linux-2.6/include/net/sctp/structs.h"
...
# 48 "linux-2.6/net/sctp/ulpqueue.c" 2
...
# 49 "linux-2.6/net/sctp/ulpqueue.c" 2
...
# 50 "linux-2.6/net/sctp/ulpqueue.c" 2
...
int sctp_clear_pd(struct sock *sk)
{
 ...
 if (!skb_queue_empty(&sp->pd_lobby)) {
  ...
  list = (struct list_head *)&(&((struct sctp_sock *)sk)->sctp)->pd_lobby;
  do { (list)->next = (list); (list)->prev = (list); } while (0);
  return 1;
 }
 return 0;
...
int sctp_ulpq_tail_event(struct sctp_ulpq *ulpq, struct sctp_ulpevent *event)
{
 ...
 if (!sctp_ulpevent_is_enabled(event, &(&((struct sctp_sock *)sk)->sctp)->subscribe))
  goto out_free;
 ...
 if (!(&((struct sctp_sock *)sk)->sctp)->pd_mode) {
  queue = &sk->sk_receive_queue;
 } else if (ulpq->pd_mode) {
  if (event->msg_flags & MSG_NOTIFICATION)
          queue = &(&((struct sctp_sock *)sk)->sctp)->pd_lobby;
  else {
      ...
      struct sctp_ulpevent *event)
{
 ...
 for (pos = (&ulpq->reasm)->next, prefetch(pos->next); (pos != (struct sk_buff *)(&ulpq->reasm)); pos = pos->next, prefetch(pos->next)) {
  cevent = sctp_skb2event(pos);
  ctsn = cevent->tsn;
...
# 292 "linux-2.6/net/sctp/ulpqueue.c"
static struct sctp_ulpevent *sctp_make_reassembled_event(struct sk_buff *f_frag, struct sk_buff *l_frag)
{
 ...
 (((*_GLOBAL_sctp_statistics_32_A[get_stack_id()])[!(((current_thread_info()->preempt_count) & (((1UL << (8))-1) << (0 + 8))))])->mibs[SCTP_MIB_REASMUSRMSGS]++);
...
static inline  __attribute__((always_inline)) struct sctp_ulpevent *sctp_ulpq_retrieve_reassembled(struct sctp_ulpq *ulpq)
{
 ...
 next_tsn = 0;
# 369 "linux-2.6/net/sctp/ulpqueue.c"
 for (pos = (&ulpq->reasm)->next, prefetch(pos->next); (pos != (struct sk_buff *)(&ulpq->reasm)); pos = pos->next, prefetch(pos->next)) {
  cevent = sctp_skb2event(pos);
  ctsn = cevent->tsn;
        ...
        struct sctp_ulpevent *event)
{
 ...
 for (pos = (&ulpq->lobby)->next, prefetch(pos->next); (pos != (struct sk_buff *)(&ulpq->lobby)); pos = pos->next, prefetch(pos->next)) {
  cevent = (struct sctp_ulpevent *) pos->cb;
  csid = cevent->stream;
  cssn = cevent->ssn;
    ...
    struct sctp_chunk *chunk, int gfp)
{
 ...
 if (!(&((struct sctp_sock *)asoc->base.sk)->sctp)->pd_mode) {
  ...
  if (event) {
   sctp_ulpq_tail_event(ulpq, event);
   (&((struct sctp_sock *)asoc->base.sk)->sctp)->pd_mode = 1;
   ulpq->pd_mode = 1;
   return;
  }
 }
        ...
        int gfp)
{
 ...
 if (chunk) {
  needed = (__builtin_constant_p((__u16)(( __u16)(__be16)(chunk->chunk_hdr->length))) ? ({ __u16 __x = ((( __u16)(__be16)(chunk->chunk_hdr->length))); ((__u16)( (((__u16)(__x) & (__u16)0x00ffU) << 8) | (((__u16)(__x) & (__u16)0xff00U) >> 8) )); }) : __fswab16((( __u16)(__be16)(chunk->chunk_hdr->length))));
...
void sctp_ulpq_abort_pd(struct sctp_ulpq *ulpq, int gfp)
{
 ...
 if (sctp_ulpevent_type_enabled(SCTP_PARTIAL_DELIVERY_EVENT,
           &(&((struct sctp_sock *)sk)->sctp)->subscribe))
