# 1 "freebsd5/netinet6/ipcomp_output.c"
...
# 41 "freebsd5/netinet6/ipcomp_output.c" 2
...
void DELAY(int usec);
# 42 "freebsd5/netinet6/ipcomp_output.c" 2
...
void *reallocf(void *addr, unsigned long size, struct malloc_type *type,
     int flags);
# 43 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 44 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 45 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 46 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 47 "freebsd5/netinet6/ipcomp_output.c" 2


# 1 "freebsd5/freebsd/usr/src/sys/sys/syslog.h" 1
# 50 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 52 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 53 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 54 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 55 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 56 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 58 "freebsd5/netinet6/ipcomp_output.c" 2
...
n_time iptime(void);
# 59 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 60 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 61 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 62 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 63 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 66 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 67 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 69 "freebsd5/netinet6/ipcomp_output.c" 2
...
extern int ipcomp6_output (struct mbuf *, u_char *, struct mbuf *, struct ipsecrequest *)
                        ;
# 71 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 74 "freebsd5/netinet6/ipcomp_output.c" 2
...
extern int ipsec6_tunnel_validate (struct mbuf *, int, u_int, struct secasvar *)
                    ;
# 76 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 78 "freebsd5/netinet6/ipcomp_output.c" 2
...
# 81 "freebsd5/netinet6/ipcomp_output.c" 2

# 1 "freebsd5/net/net_osdep.h" 1
# 322 "freebsd5/net/net_osdep.h"
struct ifnet;
extern const char *if_name(struct ifnet *);
# 83 "freebsd5/netinet6/ipcomp_output.c" 2

static int ipcomp_output (struct mbuf *, u_char *, struct mbuf *, struct ipsecrequest *, int)
                             ;
# 105 "freebsd5/netinet6/ipcomp_output.c"
static int
 ...
 int af;
{
 ...
 default:
  do { if (global_ipsec_debug[get_stack_id()]) nsc_log (3, "ipcomp_output: unsupported af %d\n", af); } while ( 0);
  return 0;
 }


 algo = ipcomp_algorithm_lookup(sav->alg_enc);
 if ((__bswap32(sav->spi) & ~0xffff) != 0 || !algo) {
  stat->out_inval++;
  m_freem(m);
  return 22;
 }
 ...
 if (plen < algo->minplen)
  return 0;
# 174 "freebsd5/netinet6/ipcomp_output.c"
 mcopy = m_copym(m, 0, 1000000000, 0x0001);
 if (mcopy == ((void *)0)) {
  error = 55;
  return 0;
 }
 md0 = m_copym(md, 0, 1000000000, 0x0001);
 if (md0 == ((void *)0)) {
  m_freem(mcopy);
  error = 55;
  return 0;
 ...
 if (mprev == ((void *)0) || mprev->m_hdr.mh_next != md) {
  do { if (global_ipsec_debug[get_stack_id()]) nsc_log (7, "ipcomp%d_output: md is not in chain\n", afnumber); } while ( 0)
                ;
  stat->out_inval++;
  m_freem(m);
  m_freem(md0);
 ...
 if ((*algo->compress)(m, md, &plen) || mprev->m_hdr.mh_next == ((void *)0)) {
  do { if (global_ipsec_debug[get_stack_id()]) nsc_log (3, "packet compression failure\n"); } while ( 0);
  m = ((void *)0);
  m_freem(md0);
  m_freem(mcopy);
  stat->out_inval++;
 ...
 m->M_dat.MH.MH_pkthdr.len -= plen0;
 ...
 compoff = m->M_dat.MH.MH_pkthdr.len - plen;
 ...
 if (((md)->m_hdr.mh_flags & 0x0001 ? ((!((md)->m_hdr.mh_flags & 0x0008) && (!((md)->m_hdr.mh_flags & 0x0001) || !(((md)->M_dat.MH.MH_dat.MH_ext.ref_cnt != ((void *)0)) && (*((md)->M_dat.MH.MH_dat.MH_ext.ref_cnt) > 1)))) ? (md)->m_hdr.mh_data - (md)->M_dat.MH.MH_dat.MH_ext.ext_buf : 0): (md)->m_hdr.mh_flags & 0x0002 ? (md)->m_hdr.mh_data - (md)->M_dat.MH.MH_dat.MH_databuf : (md)->m_hdr.mh_data - (md)->M_dat.M_databuf) < complen) {
  ((n) = m_get((0x0001), (1)));
  if (!n) {
   m_freem(m);
   error = 55;
  ...
  n->m_hdr.mh_len = complen;
  mprev->m_hdr.mh_next = n;
  n->m_hdr.mh_next = md;
  m->M_dat.MH.MH_pkthdr.len += complen;
  ipcomp = ((struct ipcomp *)((n)->m_hdr.mh_data));
 } else {
  md->m_hdr.mh_len += complen;
  md->m_hdr.mh_data -= complen;
  m->M_dat.MH.MH_pkthdr.len += complen;
 ...
 *nexthdrp = 108;
 ipcomp->comp_cpi = __bswap16(cpi);
 switch (af) {

 case 2:
  if (compoff + complen + plen < 65535)
   ip->ip_len = __bswap16(compoff + complen + plen);
  else {
 ...
 if (!m) {
  do { if (global_ipsec_debug[get_stack_id()]) nsc_log (7, "NULL mbuf after compression in ipcomp%d_output", afnumber); } while ( 0)
 ...
 struct ipsecrequest *isr;
{
 ...
 if (m->m_hdr.mh_len < sizeof(struct ip)) {
  do { if (global_ipsec_debug[get_stack_id()]) nsc_log (7, "ipcomp4_output: first mbuf too short\n"); } while ( 0);
  global_ipsecstat[get_stack_id()].out_inval++;
  m_freem(m);
  return 0;
 }
 ...
 return ipcomp_output(m, &ip->ip_p, m->m_hdr.mh_next, isr, 2);
}
 ...
 struct ipsecrequest *isr;
{
 if (m->m_hdr.mh_len < sizeof(struct ip6_hdr)) {
  do { if (global_ipsec_debug[get_stack_id()]) nsc_log (7, "ipcomp6_output: first mbuf too short\n"); } while ( 0);
  global_ipsec6stat[get_stack_id()].out_inval++;
  m_freem(m);
  return 0;
 }
 return ipcomp_output(m, nexthdrp, md, isr, 28);
}
