# 1 "freebsd5/support/default_route.c"
...
# 1 "<command-line>" 2
# 1 "freebsd5/support/default_route.c"
# 19 "freebsd5/support/default_route.c"
...
# 20 "freebsd5/support/default_route.c" 2
...
void thr_exit1(void);
# 21 "freebsd5/support/default_route.c" 2
...
# 22 "freebsd5/support/default_route.c" 2
...
# 23 "freebsd5/support/default_route.c" 2
...
# 24 "freebsd5/support/default_route.c" 2
...
# 25 "freebsd5/support/default_route.c" 2
...
# 26 "freebsd5/support/default_route.c" 2
...
# 28 "freebsd5/support/default_route.c" 2
...
# 29 "freebsd5/support/default_route.c" 2
...
# 30 "freebsd5/support/default_route.c" 2
...
# 31 "freebsd5/support/default_route.c" 2
...
# 73 "freebsd5/freebsd/usr/include/netatalk/at.h"
struct sockaddr_at {
    u_char sat_len;
    u_char sat_family;
    u_char sat_port;
...
# 33 "freebsd5/support/default_route.c" 2
# 1 "freebsd5/freebsd/usr/include/arpa/inet.h" 1
# 131 "freebsd5/freebsd/usr/include/arpa/inet.h"

# 140 "freebsd5/freebsd/usr/include/arpa/inet.h"
in_addr_t __inet_addr(const char *);
char *__inet_ntoa(struct in_addr);
const char *__inet_ntop(int, const void * restrict, char * restrict,
      socklen_t);
int __inet_pton(int, const char * restrict, void * restrict);


int ascii2addr(int, const char *, void *);
char *addr2ascii(int, const void *, int, char *);
int __inet_aton(const char *, struct in_addr *);
in_addr_t __inet_lnaof(struct in_addr);
struct in_addr __inet_makeaddr(in_addr_t, in_addr_t);
char * __inet_neta(in_addr_t, char *, size_t);
in_addr_t __inet_netof(struct in_addr);
in_addr_t __inet_network(const char *);
char *__inet_net_ntop(int, const void *, int, char *, size_t);
int __inet_net_pton(int, const char *, void *, size_t);
unsigned __inet_nsap_addr(const char *, unsigned char *, int);
char *__inet_nsap_ntoa(int, const unsigned char *, char *);


# 34 "freebsd5/support/default_route.c" 2
# 1 "freebsd5/freebsd/usr/include/netdb.h" 1
# 85 "freebsd5/freebsd/usr/include/netdb.h"
typedef  int  _GLOBAL_154_T; extern  _GLOBAL_154_T  global_h_errno[NUM_STACKS];   
...
# 205 "freebsd5/freebsd/usr/include/netdb.h"
...
int def_gw_init(unsigned int gw_addr)
{
    ...
    nsc_assert(socreate(17, &so, 3, 2, global_proc0[get_stack_id()].p_ucred, global___pcpu[get_stack_id()].pc_curthread) == 0)

      ;

    memset(&rtm_msg, 0, sizeof(rtm_msg));

    rtm_msg.msg.rtm_msglen = 128;
    rtm_msg.msg.rtm_version = 5;
    rtm_msg.msg.rtm_type = 0x1;
    rtm_msg.msg.rtm_flags = 2051;
    rtm_msg.msg.rtm_addrs = 0x1 | 0x2 | 0x4;
    rtm_msg.msg.rtm_seq = 1;
    rtm_msg.msg.rtm_inits = 0;
    ...
    gtw = (struct sockaddr_in *)&rtm_msg.space[ ((dst->sin_len) > 0 ? (1 + (((dst->sin_len) - 1) | (sizeof(long) - 1))) : sizeof(long)) ];
    gtw->sin_len = sizeof(struct sockaddr_in);
    gtw->sin_family = 2;
    gtw->sin_addr.s_addr = gw_addr;

    mask = (struct sockaddr_in *)&rtm_msg.space[ ((dst->sin_len) > 0 ? (1 + (((dst->sin_len) - 1) | (sizeof(long) - 1))) : sizeof(long)) +
        ((gtw->sin_len) > 0 ? (1 + (((gtw->sin_len) - 1) | (sizeof(long) - 1))) : sizeof(long)) ];
    mask->sin_len = sizeof(struct sockaddr_in);
    mask->sin_family = 2;

    rtm_msg.msg.rtm_msglen = sizeof(struct rt_msghdr) + ((dst->sin_len) > 0 ? (1 + (((dst->sin_len) - 1) | (sizeof(long) - 1))) : sizeof(long))*3;





    datalen = rtm_msg.msg.rtm_msglen;

    nsc_assert(nsc_sosend(so, ((void *)0), &rtm_msg, &datalen) == 0)
...
void def_gw_get()
{
    ...
    rtm_msg.msg.rtm_msglen = 168;
    rtm_msg.msg.rtm_version = 5;
    rtm_msg.msg.rtm_type = 0x4;
    rtm_msg.msg.rtm_flags = 2051;
    rtm_msg.msg.rtm_addrs = 21;
    rtm_msg.msg.rtm_seq = 2;
    rtm_msg.msg.rtm_inits = 0;
    ...
    nsc_assert(nsc_soreceive_blocking(so, buf, &buflen, ((void *)0)) == 0)

           ;

    r = (struct rt_msghdr *)buf;
    sa = (struct sockaddr *)(r + 1);
    nsc_printf("%s: read says:\n", __FUNCTION__);
    for(i = 0; i < (r->rtm_msglen - sizeof(struct rt_msghdr)); i++) {
        nsc_printf("%02hhx ", ((unsigned char *)sa)[i]);
    }
