...
# 43 "freebsd5/net/slcompress.c" 2
...
# 44 "freebsd5/net/slcompress.c" 2
...
# 47 "freebsd5/net/slcompress.c" 2
...
n_time iptime(void);
# 48 "freebsd5/net/slcompress.c" 2
...
# 49 "freebsd5/net/slcompress.c" 2
...
# 50 "freebsd5/net/slcompress.c" 2
...
# 52 "freebsd5/net/slcompress.c" 2
# 62 "freebsd5/net/slcompress.c"
void
 ...
 int compress_cid;
{
 ...
 if ((ip->ip_off & __bswap16(0x3fff)) || m->m_hdr.mh_len < 40)
  return (0x40);
...
# 258 "freebsd5/net/slcompress.c"
 ...
 if (((u_int16_t *)ip)[0] != ((u_int16_t *)&cs->slcs_u.csu_ip)[0] ||
     ...
     (deltaS > 5 &&
      bcmp((void *)(ip + 1), (void *)(&cs->slcs_u.csu_ip + 1), (int)((deltaS - 5) << 2))) ||
     (th->th_off > 5 &&
      bcmp((void *)(th + 1), (void *)(oth + 1), (int)((th->th_off - 5) << 2))))
  goto uncompressed;
 ...
 deltaS = (u_int16_t)(__bswap16(th->th_win) - __bswap16(oth->th_win));
 if (deltaS) {
  { if ((u_int16_t)(deltaS) >= 256) { *cp++ = 0; cp[1] = (deltaS); cp[0] = (deltaS) >> 8; cp += 2; } else { *cp++ = (deltaS); } };
  changes |= 0x02;
 }

 deltaA = __bswap32(th->th_ack) - __bswap32(oth->th_ack);
 if (deltaA) {
  if (deltaA > 0xffff)
   goto uncompressed;
 ...
 deltaS = __bswap32(th->th_seq) - __bswap32(oth->th_seq);
 if (deltaS) {
  if (deltaS > 0xffff)
   goto uncompressed;
  { if ((u_int16_t)(deltaS) >= 256) { *cp++ = 0; cp[1] = (deltaS); cp[0] = (deltaS) >> 8; cp += 2; } else { *cp++ = (deltaS); } };
 ...
 case 0:
# 325 "freebsd5/net/slcompress.c"
  if (ip->ip_len != cs->slcs_u.csu_ip.ip_len &&
      __bswap16(cs->slcs_u.csu_ip.ip_len) == hlen)
   break;
 ...
 case 0x08|0x04:
  if (deltaS == deltaA &&
      deltaS == __bswap16(cs->slcs_u.csu_ip.ip_len) - hlen) {
 ...
 case 0x08:
  if (deltaS == __bswap16(cs->slcs_u.csu_ip.ip_len) - hlen) {
 ...
 deltaS = __bswap16(ip->ip_id) - __bswap16(cs->slcs_u.csu_ip.ip_id);
 if (deltaS != 1) {
  { if ((u_int16_t)(deltaS) >= 256 || (u_int16_t)(deltaS) == 0) { *cp++ = 0; cp[1] = (deltaS); cp[0] = (deltaS) >> 8; cp += 2; } else { *cp++ = (deltaS); } };
  changes |= 0x20;
 }
 ...
 deltaA = __bswap16(th->th_sum);
 bcopy((void *)(ip), (void *)(&cs->slcs_u.csu_ip), (int)(hlen));
# 380 "freebsd5/net/slcompress.c"
 deltaS = cp - new_seq;
 cp = (u_char *)ip;
 ...
 *cp++ = deltaA >> 8;
 *cp++ = deltaA;
 bcopy((void *)(new_seq), (void *)(cp), (int)(deltaS));
 ++comp->sls_compressed;
 return (0x80);
 ...
 struct slcompress *comp;
{
 ...
 cp += vjlen;
 len -= vjlen;
# 442 "freebsd5/net/slcompress.c"
 if ((intptr_t)cp & 3) {
  if (len > 0)
   bcopy((void *)(cp), (void *)(((intptr_t)cp &~ 3)), (int)(len));
  cp = (u_char *)((intptr_t)cp &~ 3);
 }
 cp -= hlen;
 len += hlen;
 bcopy((void *)(hdr), (void *)(cp), (int)(hlen));

 *bufp = cp;
 return (len);
}
# 462 "freebsd5/net/slcompress.c"
int
 ...
 u_int *hlenp;
{
 ...
 case (0x08|0x02|0x01):
  {
  register u_int i = __bswap16(cs->slcs_u.csu_ip.ip_len) - cs->cs_hlen;
  th->th_ack = __bswap32(__bswap32(th->th_ack) + i);
  th->th_seq = __bswap32(__bswap32(th->th_seq) + i);
  }
  break;

 case (0x08|0x04|0x02|0x01):
  th->th_seq = __bswap32(__bswap32(th->th_seq) + __bswap16(cs->slcs_u.csu_ip.ip_len) - cs->cs_hlen)
                     ;
  break;
 ...
 default:
  if (changes & 0x01) {
   th->th_flags |= 0x20;
   { if (*cp == 0) { (th->th_urp) = __bswap16((cp[1] << 8) | cp[2]); cp += 3; } else { (th->th_urp) = __bswap16((u_int32_t)*cp++); } }
  } else
   th->th_flags &=~ 0x20;
  if (changes & 0x02)
   { if (*cp == 0) { (th->th_win) = __bswap16(__bswap16(th->th_win) + ((cp[1] << 8) | cp[2])); cp += 3; } else { (th->th_win) = __bswap16(__bswap16(th->th_win) + (u_int32_t)*cp++); } }
  if (changes & 0x04)
   { if (*cp == 0) { (th->th_ack) = __bswap32(__bswap32(th->th_ack) + ((cp[1] << 8) | cp[2])); cp += 3; } else { (th->th_ack) = __bswap32(__bswap32(th->th_ack) + (u_int32_t)*cp++); } }
  if (changes & 0x08)
   { if (*cp == 0) { (th->th_seq) = __bswap32(__bswap32(th->th_seq) + ((cp[1] << 8) | cp[2])); cp += 3; } else { (th->th_seq) = __bswap32(__bswap32(th->th_seq) + (u_int32_t)*cp++); } }
  break;
 }
 if (changes & 0x20) {
  { if (*cp == 0) { (cs->slcs_u.csu_ip.ip_id) = __bswap16(__bswap16(cs->slcs_u.csu_ip.ip_id) + ((cp[1] << 8) | cp[2])); cp += 3; } else { (cs->slcs_u.csu_ip.ip_id) = __bswap16(__bswap16(cs->slcs_u.csu_ip.ip_id) + (u_int32_t)*cp++); } }
 } else
  cs->slcs_u.csu_ip.ip_id = __bswap16(__bswap16(cs->slcs_u.csu_ip.ip_id) + 1);
 ...
 cs->slcs_u.csu_ip.ip_len = __bswap16(total_len);
