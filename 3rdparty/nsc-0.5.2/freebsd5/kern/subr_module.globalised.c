...
# 1 "<command-line>" 2
# 1 "freebsd5/kern/subr_module.c"
# 27 "freebsd5/kern/subr_module.c"
# 1 "freebsd5/freebsd/usr/src/sys/sys/cdefs.h" 1
# 28 "freebsd5/kern/subr_module.c" 2
#include "num_stacks.h"
__asm__(".ident\t\"" "$FreeBSD: src/sys/kern/subr_module.c,v 1.8 2003/06/11 00:56:57 obrien Exp $" "\"");
...
# 31 "freebsd5/kern/subr_module.c" 2
...
void DELAY(int usec);
# 32 "freebsd5/kern/subr_module.c" 2
...
typedef  linker_file_t  _GLOBAL_40_T; extern  _GLOBAL_40_T  global_linker_kernel_file[NUM_STACKS];   
...
typedef  caddr_t  _GLOBAL_41_T; extern  _GLOBAL_41_T  global_preload_metadata[NUM_STACKS];   
...
# 33 "freebsd5/kern/subr_module.c" 2





typedef  caddr_t _GLOBAL_42_T;  _GLOBAL_42_T  global_preload_metadata[NUM_STACKS];  
...
preload_search_by_name(const char *name)
{
    ...
    if (global_preload_metadata[get_stack_id()] != ((void *)0)) {

 curp = global_preload_metadata[get_stack_id()];
 for (;;) {
     hdr = (u_int32_t *)curp;
     if (hdr[0] == 0 && hdr[1] == 0)
  break;
  ...
  return(curp);
     ...
     next = ((((next)+((sizeof(u_long))-1))/(sizeof(u_long)))*(sizeof(u_long)));
     curp += next;
 }
    }
    return(((void *)0));
