...
# 1 "<command-line>" 2
# 1 "freebsd5/kern/subr_disk.c"
# 10 "freebsd5/kern/subr_disk.c"
# 1 "freebsd5/freebsd/usr/src/sys/sys/cdefs.h" 1
# 11 "freebsd5/kern/subr_disk.c" 2
#include "num_stacks.h"
__asm__(".ident\t\"" "$FreeBSD: src/sys/kern/subr_disk.c,v 1.80 2003/10/18 17:27:10 phk Exp $" "\"");

# 1 "freebsd5/freebsd/usr/obj/usr/src/sys/GENERIC/opt_geom.h" 1
# 14 "freebsd5/kern/subr_disk.c" 2
...
# 16 "freebsd5/kern/subr_disk.c" 2
...
void DELAY(int usec);
# 17 "freebsd5/kern/subr_disk.c" 2
...
# 18 "freebsd5/kern/subr_disk.c" 2
...
# 19 "freebsd5/kern/subr_disk.c" 2
# 1 "freebsd5/freebsd/usr/src/sys/sys/disk.h" 1
# 16 "freebsd5/freebsd/usr/src/sys/sys/disk.h"
# 1 "freebsd5/freebsd/usr/src/sys/sys/ioccom.h" 1
# 17 "freebsd5/freebsd/usr/src/sys/sys/disk.h" 2
...
# 20 "freebsd5/kern/subr_disk.c" 2
# 1 "freebsd5/freebsd/usr/src/sys/geom/geom_disk.h" 1
# 46 "freebsd5/freebsd/usr/src/sys/geom/geom_disk.h"
...
# 21 "freebsd5/kern/subr_disk.c" 2
# 29 "freebsd5/kern/subr_disk.c"
void
disk_err(struct bio *bp, const char *what, int blkdone, int nl)
{
 ...
 if (bp->bio_dev != ((void *)0))
  nsc_printf("%s: %s ", devtoname(bp->bio_dev), what);
 else if (bp->bio_disk != ((void *)0))
  nsc_printf("%s%d: %s ",
      bp->bio_disk->d_name, bp->bio_disk->d_unit, what);
 else
  nsc_printf("disk??: %s ", what);
 switch(bp->bio_cmd) {
 case 0x01: nsc_printf("cmd=read "); break;
 case 0x02: nsc_printf("cmd=write "); break;
 case 0x04: nsc_printf("cmd=delete "); break;
 case 0x08: nsc_printf("cmd=getattr "); break;
 default: nsc_printf("cmd=%x ", bp->bio_cmd); break;
 }
 sn = bp->bio_pblkno;
 if (bp->bio_bcount <= (1<<9)) {
  nsc_printf("fsbn %jd%s", (intmax_t)sn, nl ? "\n" : "");
  return;
 }
 if (blkdone >= 0) {
  sn += blkdone;
  nsc_printf("fsbn %jd of ", (intmax_t)sn);
 }
 nsc_printf("%jd-%jd", (intmax_t)bp->bio_pblkno,
     (intmax_t)(bp->bio_pblkno + (bp->bio_bcount - 1) / (1<<9)));
 if (nl)
  nsc_printf("\n");
}
...
{
 do { (((&head->queue))->tqh_first) = ((void *)0); (&head->queue)->tqh_last = &(((&head->queue))->tqh_first); ; } while (0);
 head->last_offset = 0;
 head->insert_point = ((void *)0);
 head->switch_point = ((void *)0);
}
...
bioq_remove(struct bio_queue_head *head, struct bio *bp)
{
 if (bp == head->switch_point)
  head->switch_point = ((bp)->bio_queue.tqe_next);
 if (bp == head->insert_point) {
  head->insert_point = (*(((struct bio_queue *)((bp)->bio_queue.tqe_prev))->tqh_last));
  if (head->insert_point == ((void *)0))
   head->last_offset = 0;
 } else if (bp == ((&head->queue)->tqh_first))
  head->last_offset = bp->bio_offset;
 do { if (((((bp))->bio_queue.tqe_next)) != ((void *)0)) (((bp))->bio_queue.tqe_next)->bio_queue.tqe_prev = (bp)->bio_queue.tqe_prev; else { (&head->queue)->tqh_last = (bp)->bio_queue.tqe_prev; ; } *(bp)->bio_queue.tqe_prev = (((bp))->bio_queue.tqe_next); ; ; ; } while (0);
 if (((&head->queue)->tqh_first) == head->switch_point)
  head->switch_point = ((void *)0);
}
...
bioq_insert_tail(struct bio_queue_head *head, struct bio *bp)
{
 ...
 do { (((bp))->bio_queue.tqe_next) = ((void *)0); (bp)->bio_queue.tqe_prev = (&head->queue)->tqh_last; *(&head->queue)->tqh_last = (bp); (&head->queue)->tqh_last = &(((bp))->bio_queue.tqe_next); ; ; } while (0);
}
...
bioq_first(struct bio_queue_head *head)
{
 ...
 return (((&head->queue)->tqh_first));
}
# 136 "freebsd5/kern/subr_disk.c"
void
 ...
 struct bio *bp;
{
 ...
 be = (*(((struct bio_queue *)((&bioq->queue)->tqh_last))->tqh_last));




 if ((bq = bioq_first(bioq)) == ((void *)0)) {
  bioq_insert_tail(bioq, bp);
  return;
 } else if (bioq->insert_point != ((void *)0)) {
  ...
  if (bp->bio_offset < bioq->last_offset) {
   ...
   if (bp->bio_offset < bq->bio_offset) {
    bioq->switch_point = bp;
    do { (bp)->bio_queue.tqe_prev = (bq)->bio_queue.tqe_prev; (((bp))->bio_queue.tqe_next) = (bq); *(bq)->bio_queue.tqe_prev = (bp); (bq)->bio_queue.tqe_prev = &(((bp))->bio_queue.tqe_next); ; ; } while (0);
    return;
   }
  } else {
   if (bioq->switch_point != ((void *)0))
    be = (*(((struct bio_queue *)((bioq->switch_point)->bio_queue.tqe_prev))->tqh_last))
                           ;
 ...
 if (bp->bio_offset > be->bio_offset) {
  do { if (((((bp))->bio_queue.tqe_next) = (((be))->bio_queue.tqe_next)) != ((void *)0)) (((bp))->bio_queue.tqe_next)->bio_queue.tqe_prev = &(((bp))->bio_queue.tqe_next); else { (&bioq->queue)->tqh_last = &(((bp))->bio_queue.tqe_next); ; } (((be))->bio_queue.tqe_next) = (bp); (bp)->bio_queue.tqe_prev = &(((be))->bio_queue.tqe_next); ; ; } while (0);
  return;
 }


 while ((bn = ((bq)->bio_queue.tqe_next)) != ((void *)0)) {
 ...
 do { if (((((bp))->bio_queue.tqe_next) = (((bq))->bio_queue.tqe_next)) != ((void *)0)) (((bp))->bio_queue.tqe_next)->bio_queue.tqe_prev = &(((bp))->bio_queue.tqe_next); else { (&bioq->queue)->tqh_last = &(((bp))->bio_queue.tqe_next); ; } (((bq))->bio_queue.tqe_next) = (bp); (bp)->bio_queue.tqe_prev = &(((bq))->bio_queue.tqe_next); ; ; } while (0);
}
