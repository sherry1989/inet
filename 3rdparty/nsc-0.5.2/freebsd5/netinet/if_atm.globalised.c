...
# 1 "<command-line>" 2
# 1 "freebsd5/netinet/if_atm.c"
# 34 "freebsd5/netinet/if_atm.c"
# 1 "freebsd5/freebsd/usr/src/sys/sys/cdefs.h" 1
# 35 "freebsd5/netinet/if_atm.c" 2
#include "num_stacks.h"
__asm__(".ident\t\"" "$FreeBSD: src/sys/netinet/if_atm.c,v 1.18 2004/08/16 18:32:07 rwatson Exp $" "\"");
...
# 48 "freebsd5/netinet/if_atm.c" 2
...
void DELAY(int usec);
# 49 "freebsd5/netinet/if_atm.c" 2
...
# 51 "freebsd5/netinet/if_atm.c" 2
...
# 52 "freebsd5/netinet/if_atm.c" 2
...
# 53 "freebsd5/netinet/if_atm.c" 2
# 1 "freebsd5/freebsd/usr/src/sys/sys/syslog.h" 1
# 54 "freebsd5/netinet/if_atm.c" 2
...
# 56 "freebsd5/netinet/if_atm.c" 2
...
# 57 "freebsd5/netinet/if_atm.c" 2
...
# 58 "freebsd5/netinet/if_atm.c" 2
...
# 59 "freebsd5/netinet/if_atm.c" 2
...
# 61 "freebsd5/netinet/if_atm.c" 2
...
int atmresolve(struct rtentry *, struct mbuf *, struct sockaddr *,
  struct atm_pseudohdr *);
# 62 "freebsd5/netinet/if_atm.c" 2
# 93 "freebsd5/netinet/if_atm.c"
void
atm_rtrequest(int req, struct rtentry *rt, struct rt_addrinfo *info)
{
 ...
 case 0xb:
  nsc_printf("atm_rtrequest: RTM_RESOLVE request detected?\n");
  break;

 case 0x1:
# 125 "freebsd5/netinet/if_atm.c"
  if ((rt->rt_flags & 0x4) == 0) {
   rt_setgate(rt,(*((struct sockaddr **)(&(rt)->rt_nodes->rn_u.rn_leaf.rn_Key))),(struct sockaddr *)&global_null_sdl[get_stack_id()]);
   gate = rt->rt_gateway;
  ...
  if ((rt->rt_flags & 0x100) != 0) {
   nsc_printf("atm_rtrequest: cloning route detected?\n");
   break;
  }
  if (gate->sa_family != 18 ||
      gate->sa_len < sizeof(global_null_sdl[get_stack_id()])) {
   nsc_log(7, "atm_rtrequest: bad gateway value");
   break;
  }
  ...
  bzero(&op, sizeof(op));
  addr = ((caddr_t)((((struct sockaddr_dl *)gate))->sdl_data + (((struct sockaddr_dl *)gate))->sdl_nlen));
  alen = ((struct sockaddr_dl *)gate)->sdl_alen;
  if (alen < 4) {
   nsc_printf("%s: bad link-level address\n", __func__);
   goto failed;
  }

  if (alen == 4) {

   do { (op.param.flags) = *(addr)++; (alen)--; } while (0);
   do { (op.param.vpi) = *(addr)++; (alen)--; } while (0);
   do { (op.param.vci) = ((addr)[0] << 8) | (addr)[1]; (addr) += 2; (alen) -= 2; } while (0);
   op.param.traffic = 0;
   op.param.aal = (op.param.flags & 0x01) ?
       5 : 0;
  } else {
   ...
   do { (op.param.flags) = *(addr)++; (alen)--; } while (0);
   op.param.flags &= 0x0002;

   do { (op.param.vpi) = *(addr)++; (alen)--; } while (0);
   do { (op.param.vci) = ((addr)[0] << 8) | (addr)[1]; (addr) += 2; (alen) -= 2; } while (0);

   do { (op.param.traffic) = *(addr)++; (alen)--; } while (0);

   switch (op.param.traffic) {

     case 0:
    if (alen >= 3)
     do { (op.param.tparam.pcr) = ((addr)[0] << 16) | ((addr)[1] << 8) | (addr)[2]; (addr) += 3; (alen) -= 3; } while (0)
                    ;
    break;
    ...
    if (alen < 3 * 3)
     goto bad_param;
    do { (op.param.tparam.pcr) = ((addr)[0] << 16) | ((addr)[1] << 8) | (addr)[2]; (addr) += 3; (alen) -= 3; } while (0);
    do { (op.param.tparam.scr) = ((addr)[0] << 16) | ((addr)[1] << 8) | (addr)[2]; (addr) += 3; (alen) -= 3; } while (0);
    do { (op.param.tparam.mbs) = ((addr)[0] << 16) | ((addr)[1] << 8) | (addr)[2]; (addr) += 3; (alen) -= 3; } while (0);
    break;
    ...
    if (alen < 4 * 3 + 2 + 1 * 2 + 3)
     goto bad_param;
    do { (op.param.tparam.pcr) = ((addr)[0] << 16) | ((addr)[1] << 8) | (addr)[2]; (addr) += 3; (alen) -= 3; } while (0);
    do { (op.param.tparam.mcr) = ((addr)[0] << 16) | ((addr)[1] << 8) | (addr)[2]; (addr) += 3; (alen) -= 3; } while (0);
    do { (op.param.tparam.icr) = ((addr)[0] << 16) | ((addr)[1] << 8) | (addr)[2]; (addr) += 3; (alen) -= 3; } while (0);
    do { (op.param.tparam.tbe) = ((addr)[0] << 16) | ((addr)[1] << 8) | (addr)[2]; (addr) += 3; (alen) -= 3; } while (0);
    do { (op.param.tparam.nrm) = *(addr)++; (alen)--; } while (0);
    do { (op.param.tparam.trm) = *(addr)++; (alen)--; } while (0);
    do { (op.param.tparam.adtf) = ((addr)[0] << 8) | (addr)[1]; (addr) += 2; (alen) -= 2; } while (0);
    do { (op.param.tparam.rif) = *(addr)++; (alen)--; } while (0);
    do { (op.param.tparam.rdf) = *(addr)++; (alen)--; } while (0);
    do { (op.param.tparam.cdf) = *(addr)++; (alen)--; } while (0);
    break;

     default:
     bad_param:
    nsc_printf("%s: bad traffic params\n", __func__);
    goto failed;
   }
  }
  op.param.rmtu = op.param.tmtu = rt->rt_ifp->if_data.ifi_mtu;
# 242 "freebsd5/netinet/if_atm.c"
  op.rxhand = ((void *)0);
  op.param.flags |= 0x0800;
  if (rt->rt_ifp->if_ioctl(rt->rt_ifp, ((unsigned long)(0x40000000 | ((sizeof(struct atmio_openvcc) & 0x1fff) << 16) | ((('a')) << 8) | ((126)))),
      (caddr_t)&op) != 0) {
   nsc_printf("atm: couldn't add VC\n");
   goto failed;
  }
...
failed:
# 266 "freebsd5/netinet/if_atm.c"
  rt->rt_flags |= 0x8;
  break;

 case 0x2:
# 286 "freebsd5/netinet/if_atm.c"
  bzero(&op, sizeof(op));
  addr = ((caddr_t)((((struct sockaddr_dl *)gate))->sdl_data + (((struct sockaddr_dl *)gate))->sdl_nlen));
  addr++;
  cl.vpi = *addr++;
  cl.vci = *addr++ << 8;
  cl.vci |= *addr++;
  (void)rt->rt_ifp->if_ioctl(rt->rt_ifp, ((unsigned long)(0x40000000 | ((sizeof(struct atmio_closevcc) & 0x1fff) << 16) | ((('a')) << 8) | ((127)))),
      (caddr_t)&cl);
  break;
 }
}
# 313 "freebsd5/netinet/if_atm.c"
int
    ...
    struct atm_pseudohdr *desten)
{
 ...
 if (m->m_hdr.mh_flags & (0x0200 | 0x0400)) {
  nsc_log(6, "atmresolve: BCAST/MCAST packet detected/dumped");
  goto bad;
 }
...
# 346 "freebsd5/netinet/if_atm.c"
 ...
 if (sdl->sdl_family == 18 && sdl->sdl_alen >= sizeof(*desten)) {
  bcopy(((caddr_t)((sdl)->sdl_data + (sdl)->sdl_nlen)), desten, sizeof(*desten));
  return (1);
 }
