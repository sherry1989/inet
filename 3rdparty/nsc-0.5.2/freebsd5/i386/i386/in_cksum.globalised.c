# 1 "freebsd5/i386/i386/in_cksum.c"
...
# 1 "<command-line>" 2
# 1 "freebsd5/i386/i386/in_cksum.c"
# 41 "freebsd5/i386/i386/in_cksum.c"
...
# 42 "freebsd5/i386/i386/in_cksum.c" 2
...
void DELAY(int usec);
# 43 "freebsd5/i386/i386/in_cksum.c" 2
...
# 44 "freebsd5/i386/i386/in_cksum.c" 2

# 1 "freebsd5/netinet/in.h" 1
# 75 "freebsd5/netinet/in.h"
...
# 46 "freebsd5/i386/i386/in_cksum.c" 2
...
n_time iptime(void);
# 47 "freebsd5/i386/i386/in_cksum.c" 2
...
# 48 "freebsd5/i386/i386/in_cksum.c" 2
...
# 50 "freebsd5/i386/i386/in_cksum.c" 2
# 85 "freebsd5/i386/i386/in_cksum.c"
u_short
 ...
 int skip;
{
 ...
 for (; skip && m; m = m->m_hdr.mh_next) {
  if (m->m_hdr.mh_len > skip) {
   ...
   w = (u_short *)(((u_char *)((m)->m_hdr.mh_data)) + skip);
   goto skip_start;
  } else {
   skip -= m->m_hdr.mh_len;
  }
 ...
 for (;m && len; m = m->m_hdr.mh_next) {
  ...
  if (mlen == -1) {
# 122 "freebsd5/i386/i386/in_cksum.c"
   su.c[1] = *(u_char *)w;
   sum += su.s;
   w = (u_short *)((char *)w + 1);
...
skip_start:
  ...
  if (3 & (int) w) {
   {sum = (sum & 0xffff) + (sum >> 16); if ((sum) > 0xffff) (sum) -= 0xffff;};
   if ((1 & (int) w) && (mlen > 0)) {
    sum <<= 8;
    su.c[0] = *(char *)w;
    w = (u_short *)((char *)w + 1);
  ...
  if (4 & (int) w && mlen >= 4) {
   __asm volatile ("addl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[0 / 4]));
   __asm volatile ("adcl         $0, %0" : "+r" (sum));
   w += 2;
   mlen -= 4;
  }
  if (8 & (int) w && mlen >= 8) {
   __asm volatile ("addl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[0 / 4]));
   __asm volatile ("adcl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[4 / 4]));
   __asm volatile ("adcl         $0, %0" : "+r" (sum));
   w += 4;
   mlen -= 8;
  }
  ...
  mlen -= 1;
  while ((mlen -= 32) >= 0) {
# 193 "freebsd5/i386/i386/in_cksum.c"
   __asm volatile ("addl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[16 / 4]));
   __asm volatile ("adcl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[0 / 4]));
   __asm volatile ("adcl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[4 / 4]));
   __asm volatile ("adcl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[8 / 4]));
   __asm volatile ("adcl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[12 / 4]));
   __asm volatile ("" : : "r" (((const u_int32_t *)w)[32 / 4]));
   __asm volatile ("adcl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[20 / 4]));
   __asm volatile ("adcl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[24 / 4]));
   __asm volatile ("adcl %1, %0" : "+r" (sum) : "g" (((const u_int32_t *)w)[28 / 4]));
   __asm volatile ("adcl         $0, %0" : "+r" (sum));
   w += 16;
  }
  mlen += 32 + 1;
