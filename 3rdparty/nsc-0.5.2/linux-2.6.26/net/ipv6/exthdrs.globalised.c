...
static int ipv6_destopt_rcv(struct sk_buff *skb)
{
 ...
 (({ (void)((0)); (((_idev)->stats.ipv6[0])); })->mibs[(IPSTATS_MIB_INHDRERRORS)]++); (({ (void)((0)); (((*_GLOBAL_ipv6_statistics_12_A[get_stack_id()])[0])); })->mibs[(IPSTATS_MIB_INHDRERRORS)]++); });
  kfree_skb(skb);
  return -1;
 }
...
static int ipv6_rthdr_rcv(struct sk_buff *skb)
{
 ...
 if (ipv6_addr_is_multicast(&ipv6_hdr(skb)->daddr) ||
     skb->pkt_type != 0) {
  ({ struct inet6_dev *_idev = (ip6_dst_idev(skb->dst)); if (__builtin_expect(!!(_idev != ((void *)0)), 1))
 (({ (void)((0)); (((_idev)->stats.ipv6[0])); })->mibs[(IPSTATS_MIB_INADDRERRORS)]++); (({ (void)((0)); (((*_GLOBAL_ipv6_statistics_12_A[get_stack_id()])[0])); })->mibs[(IPSTATS_MIB_INADDRERRORS)]++); });
  kfree_skb(skb);
  return -1;
 }
...
# 394 "linux-2.6.26/net/ipv6/exthdrs.c"
 ...
 if (skb_cloned(skb)) {
  ...
  if (pskb_expand_head(skb, 0, 0, ((( gfp_t)0x20u)))) {
   ({ struct inet6_dev *_idev = (ip6_dst_idev(skb->dst)); if (__builtin_expect(!!(_idev != ((void *)0)), 1))
 (({ (void)((0)); (((_idev)->stats.ipv6[0])); })->mibs[(IPSTATS_MIB_OUTDISCARDS)]++); (({ (void)((0)); (((*_GLOBAL_ipv6_statistics_12_A[get_stack_id()])[0])); })->mibs[(IPSTATS_MIB_OUTDISCARDS)]++); });
   kfree_skb(skb);
   return -1;
  }
  hdr = (struct ipv6_rt_hdr *)skb_transport_header(skb);
