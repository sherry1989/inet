# 1 "openbsd3/altq/altq_cbq.c"
# 1 "/home/cmcc/omnet_workspace/inet/3rdparty/nsc-0.5.2//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "openbsd3/altq/altq_cbq.c"
# 34 "openbsd3/altq/altq_cbq.c"
...
# 39 "openbsd3/altq/altq_cbq.c" 2
# 1 "openbsd3/sys/errno.h" 1
# 40 "openbsd3/altq/altq_cbq.c" 2
...
# 47 "openbsd3/altq/altq_cbq.c" 2
# 1 "openbsd3/altq/altq_cbq.h" 1
# 38 "openbsd3/altq/altq_cbq.h"
...
# 41 "openbsd3/altq/altq_cbq.h" 2
# 68 "openbsd3/altq/altq_cbq.h"
...
# 109 "openbsd3/altq/altq_cbq.h"
typedef struct cbqstate {
 int cbq_qlen;
 struct rm_class *cbq_class_tbl[256];

 struct rm_ifdat ifnp;
 struct callout cbq_callout;
} cbq_state_t;
# 48 "openbsd3/altq/altq_cbq.c" 2
...
get_class_stats(class_stats_t *statsp, struct rm_class *cl)
{
 ...
 statsp->qmax = (cl->q_)->qlim_;
 statsp->ns_per_byte = cl->ns_per_byte_;
 statsp->wrr_allot = cl->w_allotment_;
 statsp->qcnt = (cl->q_)->qlen_;
 statsp->avgidle = cl->avgidle_;

 statsp->qtype = (cl->q_)->qtype_;
...
cbq_pfattach(struct pf_altq *a)
{
 ...
 if ((ifp = ifunit(a->ifname)) == 0L || a->altq_disc == 0L)
  return (22);
 s = splraise((0x20 + ((6) << 4)));
 error = altq_attach(&ifp->if_snd, 1, a->altq_disc,
     cbq_enqueue, cbq_dequeue, cbq_request, 0L, 0L);
 splx(s);
 return (error);
}
...
cbq_add_altq(struct pf_altq *a)
{
 ...
 (cbqp) = (cbq_state_t *)malloc((u_long)(sizeof(cbq_state_t)), 2, 0x0000);
 if (cbqp == 0L)
  return (12);
 bzero(cbqp, sizeof(cbq_state_t));
 do { bzero((&cbqp->cbq_callout), sizeof(*(&cbqp->cbq_callout))); } while (0);
 cbqp->cbq_qlen = 0;
...
cbq_add_queue(struct pf_altq *a)
{
 ...
 if (parent == 0L && (opts->flags & 0x1000) == 0) {
  printf("cbq_add_queue: no parent class!\n");
  return (22);
 }

 if ((borrow != parent) && (borrow != 0L)) {
...
cbq_remove_queue(struct pf_altq *a)
{
 ...
 if ((cl = clh_to_clp(cbqp, a->qid)) == 0L)
  return (22);
...
cbq_getqstats(struct pf_altq *a, void *ubuf, int *nbytes)
{
 ...
 if ((cbqp = altq_lookup(a->ifname, 1)) == 0L)
  return (9);
...
cbq_enqueue(struct ifaltq *ifq, struct mbuf *m, struct altq_pktattr *pktattr)
{
 ...
 if (t == 0L ||
     (cl = clh_to_clp(cbqp, ((struct altq_tag *)(t+1))->qid)) == 0L) {
 ...
 if (rmc_queue_packet(cl, m) != 0) {
  ...
  do { (&cl->stats_.drop_cnt)->packets++; (&cl->stats_.drop_cnt)->bytes += len; } while (0);
  return (55);
 }
...
cbqrestart(struct ifaltq *ifq)
{
 ...
 if ((cbqp = (cbq_state_t *)ifq->altq_disc) == 0L)

  return;

 ifp = ifq->altq_ifp;
 if (ifp->if_start &&
     cbqp->cbq_qlen > 0 && (ifp->if_flags & 0x400) == 0)
  (*ifp->if_start)(ifp);
}
...
static void cbq_purge(cbq_state_t *cbqp)
{
 ...
 for (i = 0; i < 256; i++)
  if ((cl = cbqp->cbq_class_tbl[i]) != 0L)
   rmc_dropall(cl);
 if (((cbqp->ifnp.ifq_)->altq_flags & 0x02))
