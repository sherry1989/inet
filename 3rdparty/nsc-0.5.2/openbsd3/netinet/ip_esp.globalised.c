# 1 "openbsd3/netinet/ip_esp.c"
# 1 "/home/cmcc/omnet_workspace/inet/3rdparty/nsc-0.5.2//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "openbsd3/netinet/ip_esp.c"
# 38 "openbsd3/netinet/ip_esp.c"
...
# 39 "openbsd3/netinet/ip_esp.c" 2
...
# 40 "openbsd3/netinet/ip_esp.c" 2
...
# 41 "openbsd3/netinet/ip_esp.c" 2
...
# 42 "openbsd3/netinet/ip_esp.c" 2
...
# 44 "openbsd3/netinet/ip_esp.c" 2
...
# 45 "openbsd3/netinet/ip_esp.c" 2
...
# 47 "openbsd3/netinet/ip_esp.c" 2
...
# 50 "openbsd3/netinet/ip_esp.c" 2
...
n_time iptime(void);
# 51 "openbsd3/netinet/ip_esp.c" 2
...
# 52 "openbsd3/netinet/ip_esp.c" 2
...
# 59 "openbsd3/netinet/ip_esp.c" 2
...
extern int ipsp_aux_match(struct ipsec_ref *, struct ipsec_ref *,
    ...
    struct sockaddr_encap *, struct sockaddr_encap *,
    struct sockaddr_encap *, struct sockaddr_encap *);
# 62 "openbsd3/netinet/ip_esp.c" 2
...
typedef  int  _GLOBAL_126_T; extern  _GLOBAL_126_T  global_esp_enable[NUM_STACKS];   
typedef  int  _GLOBAL_127_T; extern  _GLOBAL_127_T  global_udpencap_enable[NUM_STACKS];   
typedef  int  _GLOBAL_128_T; extern  _GLOBAL_128_T  global_udpencap_port[NUM_STACKS];   
typedef  struct espstat   _GLOBAL_129_T; extern  _GLOBAL_129_T  global_espstat[NUM_STACKS];    
# 63 "openbsd3/netinet/ip_esp.c" 2
...
# 64 "openbsd3/netinet/ip_esp.c" 2
...
# 65 "openbsd3/netinet/ip_esp.c" 2
...
# 67 "openbsd3/netinet/ip_esp.c" 2
...
typedef  struct espstat  _GLOBAL_150_T;  _GLOBAL_150_T  global_espstat[NUM_STACKS];   
...
esp_init(struct tdb *tdbp, struct xformsw *xsp, struct ipsecinit *ii)
{
 ...
 if (ii->ii_encalg) {
  switch (ii->ii_encalg) {
  case 2:
   txform = &(*_GLOBAL_enc_xform_des_32_A[get_stack_id()]);
   break;

  case 3:
   txform = &(*_GLOBAL_enc_xform_3des_33_A[get_stack_id()]);
   break;
  ...
  case 7:
   txform = &(*_GLOBAL_enc_xform_blf_34_A[get_stack_id()]);
   break;

  case 6:
   txform = &(*_GLOBAL_enc_xform_cast5_35_A[get_stack_id()]);
   break;

  case 249:
   txform = &(*_GLOBAL_enc_xform_skipjack_36_A[get_stack_id()]);
   break;
 ...
 if (tdbp->tdb_encalgxform) {
  ...
  (tdbp->tdb_emxkey) = (u_int8_t *)malloc((u_long)(tdbp->tdb_emxkeylen), 76, 0x0000)
                        ;
...
esp_zeroize(struct tdb *tdbp)
{
 ...
 if (tdbp->tdb_emxkey) {
  ...
  free((caddr_t)(tdbp->tdb_emxkey), 76);
  tdbp->tdb_emxkey = 0L;
 }
...
esp_input(struct mbuf *m, struct tdb *tdb, int skip, int protoff)
{
 ...
 if (plen <= 0) {
  ;
  global_espstat[get_stack_id()].esps_badilen++;
  m_freem(m);
  return 22;
 }
 ...
 if ((tdb->tdb_wnd > 0) && (!(tdb->tdb_flags & 0x02000))) {
  ...
  case 1:
   m_freem(m);
   ;
   global_espstat[get_stack_id()].esps_wrap++;
   return 13;
  ...
  default:
   m_freem(m);
   ;
   global_espstat[get_stack_id()].esps_replay++;
   return 13;
  }
 }
 ...
 global_espstat[get_stack_id()].esps_ibytes += m->M_dat.MH.MH_pkthdr.len - skip - hlen - alen;
 ...
 if (crp == 0L) {
  m_freem(m);
  ;
  global_espstat[get_stack_id()].esps_crypto++;
  return 55;
 }
 ...
 if (esph == 0L || mtag != 0L)
  ...
  (tc) = (struct tdb_crypto *)malloc((u_long)(sizeof(struct tdb_crypto) + alen), 76, 0x0001)
                                                          ;
 if (tc == 0L) {
  m_freem(m);
  crypto_freereq(crp);
 ...
 if (espx) {
  ...
  if (tdb->tdb_flags & 0x00040) {
   ...
   m_copydata(m, crde->crd_inject, tdb->tdb_ivlen, crde->CRD_INI.cri_iv);


   for (btsx = 0; btsx < tdb->tdb_ivlen; btsx++)
    crde->CRD_INI.cri_iv[tdb->tdb_ivlen + btsx] = ~crde->CRD_INI.cri_iv[btsx];
...
esp_input_cb(void *op)
{
 ...
 if (tdb == 0L) {
  free((caddr_t)(tc), 76);
  global_espstat[get_stack_id()].esps_notdb++;
  ;
  error = 1;
  goto baddone;
 }
 ...
 if (crp->crp_etype) {
  ...
  free((caddr_t)(tc), 76);
  global_espstat[get_stack_id()].esps_noxform++;
  ;
  error = crp->crp_etype;
  goto baddone;
 }
 ...
 if (esph != 0L) {
  ...
  if (mtag == 0L) {
   ...
   if (bcmp(ptr, aalg, esph->authsize)) {
    free((caddr_t)(tc), 76);
    ;
    global_espstat[get_stack_id()].esps_badauth++;
    error = 13;
    goto baddone;
   }
  }
 ...
 if (m1 == 0L) {
  global_espstat[get_stack_id()].esps_hdrops++;
  splx(s);
 ...
 if (!(tdb->tdb_flags & 0x04000)) {
  if ((lastthree[1] != lastthree[0]) && (lastthree[1] != 0)) {
   global_espstat[get_stack_id()].esps_badenc++;
   splx(s);
   ;
   m_freem(m);
   return 22;
    ...
    int protoff)
{
 ...
 struct mbuf *mi, *mo = (struct mbuf *) 0L;
 struct tdb_crypto *tc;
 unsigned char *pad;
 u_int8_t prot;
 ...
 global_espstat[get_stack_id()].esps_output++;
 ...
 case 2:
  ...
  if (skip + hlen + rlen + padding + alen > 65535) {
                            ...
                            ;
   m_freem(m);
   global_espstat[get_stack_id()].esps_toobig++;
   return 40;
  }
  break;
 ...
 default:
                                                         ...
                                                         ;
  m_freem(m);
  global_espstat[get_stack_id()].esps_nopf++;
  return 46;
 }


 tdb->tdb_cur_bytes += m->M_dat.MH.MH_pkthdr.len - skip;
 global_espstat[get_stack_id()].esps_obytes += m->M_dat.MH.MH_pkthdr.len - skip;
 ...
 bcopy((caddr_t) &tdb->tdb_spi, ((caddr_t)((mo)->m_hdr.mh_data)), sizeof(u_int32_t));
 if (!(tdb->tdb_flags & 0x02000)) {
  u_int32_t replay = __extension__({ u_int32_t __swap32_x = (tdb->tdb_rpl++); __builtin_constant_p(tdb->tdb_rpl++) ? __extension__({ u_int32_t __swap32gen_x = (__swap32_x); (u_int32_t)((__swap32gen_x & 0xff) << 24 | (__swap32gen_x & 0xff00) << 8 | (__swap32gen_x & 0xff0000) >> 8 | (__swap32gen_x & 0xff000000) >> 24); }) : ({ u_int32_t __swap32md_x = (__swap32_x); __asm ("bswap %1" : "+r" (__swap32md_x)); __swap32md_x; }); });
  bcopy((caddr_t) &replay, ((caddr_t)((mo)->m_hdr.mh_data)) + sizeof(u_int32_t),
      sizeof(u_int32_t));
 }
 ...
 if (espx) {
  ...
  if (tdb->tdb_flags & 0x00040) {
   ...
   bcopy(tdb->tdb_iv, crde->CRD_INI.cri_iv, tdb->tdb_ivlen);
   for (ilen = 0; ilen < tdb->tdb_ivlen; ilen++)
    crde->CRD_INI.cri_iv[tdb->tdb_ivlen + ilen] =
        ~crde->CRD_INI.cri_iv[ilen];
...
esp_output_cb(void *op)
{
 ...
 if (tdb->tdb_flags & 0x00040)
  m_copydata(m, m->M_dat.MH.MH_pkthdr.len - tdb->tdb_ivlen, tdb->tdb_ivlen,
      tdb->tdb_iv);
...
m_pad(struct mbuf *m, int n)
{
 ...
 while (m0->m_hdr.mh_len < len) {
  len -= m0->m_hdr.mh_len;
  m0 = m0->m_hdr.mh_next;
 }

 if (m0->m_hdr.mh_len != len) {
 ...
 for (m1 = m0; m1->m_hdr.mh_next; m1 = m1->m_hdr.mh_next) {
  if (m1->m_hdr.mh_next->m_hdr.mh_len != 0) {
 ...
 if ((m0->m_hdr.mh_flags & 0x0001) ||
     m0->m_hdr.mh_data + m0->m_hdr.mh_len + pad >= &(m0->M_dat.M_databuf[(256 - sizeof(struct m_hdr))])) {

  do { { int ms = splraise((0x20 + ((6) << 4))); { (m1) = pool_get(&global_mbpool[get_stack_id()], (0x0001) == 0x0000 ? 0x02|0x20 : 0); } splx(ms); }; if (m1) { (m1)->m_hdr.mh_type = (1); { int ms = splraise((0x20 + ((6) << 4))); { global_mbstat[get_stack_id()].m_mtypes[1]++; } splx(ms); } (m1)->m_hdr.mh_next = (struct mbuf *)0L; (m1)->m_hdr.mh_nextpkt = (struct mbuf *)0L; (m1)->m_hdr.mh_data = (m1)->M_dat.M_databuf; (m1)->m_hdr.mh_flags = 0; } } while( 0);
  if (m1 == 0) {
   m_freem(m0);
   ;
   return 0L;
 ...
 retval = m0->m_hdr.mh_data + m0->m_hdr.mh_len;
 m0->m_hdr.mh_len += pad;
 m->M_dat.MH.MH_pkthdr.len += pad;
