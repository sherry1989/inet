# 1 "openbsd3/support/socket.c"
# 1 "/home/cmcc/omnet_workspace/inet/3rdparty/nsc-0.5.2//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "openbsd3/support/socket.c"
# 20 "openbsd3/support/socket.c"
...
# 21 "openbsd3/support/socket.c" 2
...
# 22 "openbsd3/support/socket.c" 2
...
# 23 "openbsd3/support/socket.c" 2
...
# 24 "openbsd3/support/socket.c" 2
...
# 25 "openbsd3/support/socket.c" 2
...
# 26 "openbsd3/support/socket.c" 2
...
# 27 "openbsd3/support/socket.c" 2
...
# 29 "openbsd3/support/socket.c" 2
...
# 30 "openbsd3/support/socket.c" 2
...
void pfctlinput(int, struct sockaddr *);
# 31 "openbsd3/support/socket.c" 2
...
int recvit(struct proc *, int, struct msghdr *, caddr_t,
      register_t *);
# 32 "openbsd3/support/socket.c" 2
...
int cpu_coredump(struct proc *, struct vnode *, struct ucred *,
     struct core *);
# 33 "openbsd3/support/socket.c" 2
# 1 "openbsd3/sys/unpcb.h" 1
# 60 "openbsd3/sys/unpcb.h"
struct unpcbid {
 uid_t unp_euid;
 gid_t unp_egid;
};
...
struct unpcb {
 ...
 struct unpcb *unp_refs;
 struct unpcb *unp_nextref;
 struct mbuf *unp_addr;
 int unp_flags;
 struct unpcbid unp_connid;
 int unp_cc;
 int unp_mbcnt;
 struct timespec unp_ctime;
};
# 34 "openbsd3/support/socket.c" 2
# 1 "openbsd3/sys/un.h" 1
# 41 "openbsd3/sys/un.h"
struct sockaddr_un {
...
# 35 "openbsd3/support/socket.c" 2
...
# 37 "openbsd3/support/socket.c" 2
...
# 39 "openbsd3/support/socket.c" 2
...
# 40 "openbsd3/support/socket.c" 2
...
void nsc_soconnect(void *so, int port, unsigned int addr)
{
    ...
    m = m_get(0x0000, 3);
    ((slen < (256 - sizeof(struct m_hdr))) ? (void)0 : __assert("", "openbsd3/support/socket.c", 54, "slen < MLEN"));

    m->m_hdr.mh_len = slen;

    memcpy(((caddr_t)((m)->m_hdr.mh_data)), &s_dest, slen);


    ((struct socket *)so)->so_rcv.sb_flags |= 0x04;
    ((struct socket *)so)->so_snd.sb_flags |= 0x04;
...
{
    struct socket *so;
    ((socreate(2, &so, 1, 6) == 0) ? (void)0 : __assert("",

 "openbsd3/support/socket.c"
# 70 "openbsd3/support/socket.c"
    ,

 72
# 70 "openbsd3/support/socket.c"
    , "socreate(AF_INET, &so, SOCK_STREAM, IPPROTO_TCP) == 0"))
        ...
        const void *data, unsigned int datalen)
{
    ...
    auio.uio_procp = global_curproc[get_stack_id()];
    ...
    if(dest) {
        addr = m_get(0x0000, 1);
        memcpy(((void *)((addr)->m_hdr.mh_data)), dest, sizeof(*dest));
    } else
        addr = 0L;

    error = sosend(so, addr, &auio, 0L, 0L, 0);
        ...
        const void *data, unsigned int datalen)
{
    ...
    so->so_rcv.sb_flags |= 0x04;
    so->so_snd.sb_flags |= 0x04;
    ...
    if(error == 0 || (error == 35 && auio.uio_offset > 0))
        return auio.uio_offset;
...
{
    struct socket *so = (struct socket *)socket;
    return (so->so_state & 0x002) > 0;
}

int nsc_islistening(void *socket)
{
    struct socket *so = (struct socket *)socket;
    return (so->so_options & 0x0002) > 0;
}
...
void nsc_solisten(void *socket, int port)
{
    ...
    nam.sin_family = 2;
    nam.sin_port = __extension__({ u_int16_t __swap16_x = ((unsigned short)port); __builtin_constant_p((unsigned short)port) ? __extension__({ u_int16_t __swap16gen_x = (__swap16_x); (u_int16_t)((__swap16gen_x & 0xff) << 8 | (__swap16gen_x & 0xff00) >> 8); }) : ({ u_int16_t __swap16md_x = (__swap16_x); __asm ("rorw $8, %w1" : "+r" (__swap16md_x)); __swap16md_x; }); });
    nam.sin_addr.s_addr = ((u_int32_t) __extension__({ u_int32_t __swap32_x = ((u_int32_t)(0x00000000)); __builtin_constant_p((u_int32_t)(0x00000000)) ? __extension__({ u_int32_t __swap32gen_x = (__swap32_x); (u_int32_t)((__swap32gen_x & 0xff) << 24 | (__swap32gen_x & 0xff00) << 8 | (__swap32gen_x & 0xff0000) >> 8 | (__swap32gen_x & 0xff000000) >> 24); }) : ({ u_int32_t __swap32md_x = (__swap32_x); __asm ("bswap %1" : "+r" (__swap32md_x)); __swap32md_x; }); }));

    m = m_get(0x0000, 3);
    m->m_hdr.mh_len = nam.sin_len;
    memcpy(((void *)((m)->m_hdr.mh_data)), &nam, sizeof(nam));
    ...
    error = sobind(so, m);
    ((error == 0) ? (void)0 : __assert("", "openbsd3/support/socket.c", 264, "error == 0"));

    error = solisten(so, 5);
    ((error == 0) ? (void)0 : __assert("", "openbsd3/support/socket.c", 267, "error == 0"));
}
...
int nsc_soaccept(void *socket, void **asock)
{
    ...
    while (head->so_qlen == 0 && head->so_error == 0) {
        ...
        error = ltsleep(&head->so_timeo, 24 | 0x100, (*_GLOBAL_netcon_27_A[get_stack_id()]), 0, 0L);
        if (error) {
            goto bad;
        }
    }
    ...
    *asock = so;
    ...
    if ((&head->so_rcv.sb_sel.si_note) != 0L) knote(&head->so_rcv.sb_sel.si_note, 0);
...
typedef  int   ( *_GLOBAL_123_T  )  ( struct mbuf   *  , int   ) ; extern  _GLOBAL_123_T global_altq_input[NUM_STACKS];      
...
typedef  struct ifnet_head   _GLOBAL_124_T; extern  _GLOBAL_124_T  global_ifnet[NUM_STACKS];    
typedef  struct ifnet   _GLOBAL_125_T; extern  _GLOBAL_125_T  * *  global_ifindex2ifnet[NUM_STACKS];    
typedef  struct ifnet   _GLOBAL_126_T; extern  _GLOBAL_126_T  * global_lo0ifp[NUM_STACKS];    
typedef  int  _GLOBAL_127_T; extern  _GLOBAL_127_T  global_if_indexlim[NUM_STACKS];   
...
# 328 "openbsd3/support/socket.c" 2
...
typedef  struct radix_mask { short  rm_b ;  char  rm_unused ;   u_char  rm_flags ;   struct radix_mask   * rm_mklist  ;   union { caddr_t  rmu_mask ;  struct radix_node   * rmu_leaf  ;   }   rm_rmu ;   int  rm_refs ;   }   _GLOBAL_128_T; extern  _GLOBAL_128_T  * global_rn_mkfreelist[NUM_STACKS];    
...
typedef  struct route_cb   _GLOBAL_129_T; extern  _GLOBAL_129_T  global_route_cb[NUM_STACKS];    
typedef  struct rtstat   _GLOBAL_130_T; extern  _GLOBAL_130_T  global_rtstat[NUM_STACKS];    
typedef  struct radix_node_head   _GLOBAL_131_T; extern  _GLOBAL_131_T  * _GLOBAL_0_rt_tables_I [ ]  ; extern  _GLOBAL_131_T  * _GLOBAL
inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_1_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_2_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_3_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_4_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_5_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_6_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_7_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_8_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_9_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_10_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_11_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_12_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_13_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_14_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_15_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_16_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_17_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_18_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_19_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_20_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_21_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_22_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_23_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_24_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_25_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_26_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_27_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_28_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_29_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_30_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_31_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_32_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_33_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_34_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_35_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_36_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_37_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_38_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_39_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_40_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_41_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_42_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_43_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_44_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_45_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_46_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_47_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_48_inet6ctlerrmap_I [ ] ; extern  _GLOBAL_139_T  _GLOBAL_49_inet6ctlerrmap_I [ ] ; static __attribute__ ((unused)) __typeof__(_GLOBAL_0_inet6ctlerrmap_I) *_GLOBAL_inet6ctlerrmap_32_A[NUM_STACKS];  
typedef  unsigned long   _GLOBAL_140_T; extern  _GLOBAL_140_T  global_in6_maxmtu[NUM_STACKS];    
...
# 332 "openbsd3/support/socket.c" 2
...
# 333 "openbsd3/support/socket.c" 2
...
# 334 "openbsd3/support/socket.c" 2
...
# 335 "openbsd3/support/socket.c" 2
...
# 336 "openbsd3/support/socket.c" 2
# 1 "openbsd3/netinet/tcp_fsm.h" 1
# 337 "openbsd3/support/socket.c" 2
...
# 340 "openbsd3/support/socket.c" 2
...
# 341 "openbsd3/support/socket.c" 2
...
typedef  struct tcp_debug   _GLOBAL_232_T; extern  _GLOBAL_232_T  _GLOBAL_0_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_1_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_2_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_3_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_4_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_5_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_6_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_7_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_8_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_9_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_10_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_11_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_12_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_13_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_14_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_15_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_16_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_17_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_18_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_19_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_20_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_21_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_22_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_23_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_24_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_25_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_26_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_27_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_28_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_29_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_30_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_31_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_32_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_33_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_34_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_35_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_36_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_37_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_38_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_39_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_40_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_41_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_42_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_43_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_44_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_45_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_46_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_47_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_48_tcp_debug_I [ ] ; extern  _GLOBAL_232_T  _GLOBAL_49_tcp_debug_I [ ] ; static __attribute__ ((unused)) __typeof__(_GLOBAL_0_tcp_debug_I) *_GLOBAL_tcp_debug_50_A[NUM_STACKS];   
typedef  int  _GLOBAL_233_T; extern  _GLOBAL_233_T  global_tcp_debx[NUM_STACKS];   
# 342 "openbsd3/support/socket.c" 2

typedef  int  _GLOBAL_234_T; extern  _GLOBAL_234_T  global_hz[NUM_STACKS];   
...
int nsc_get_tcp_var(void *socket, const char *var, char *result, int rlen)
{
    ...
    {
        snprintf(result, rlen, "%f", (float)tp->t_srtt / (float)8
                );
        return 1;
    }
...
int nsc_setsockopt(void *socket, char *optname, void *val, size_t valsize)
{
    ...
    if(valsize) {
        m = m_get(0x0000, 4);
        if(valsize > (256 - sizeof(struct m_hdr))) {
        ...
        err = copyin(val, ((caddr_t)((m)->m_hdr.mh_data)), valsize);
        m->m_hdr.mh_len = valsize;
    }
...
int nsc_getsockpeername(void *socket, struct nsc_sockaddr *sa, int getpeername)
{
 ...
 sa_kern = ((struct sockaddr*)((m)->m_hdr.mh_data));
